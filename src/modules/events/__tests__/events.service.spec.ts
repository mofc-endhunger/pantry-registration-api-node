...
describe('EventsService', () => {
  ...
  it('createTimeslot maps dto and saves', async () => {
    ...
    const res = await service.createTimeslot({
      event_id: 1,
      start_at: '2025-01-01',
      end_at: '2025-01-01',
      capacity: 5,
  ...
  it('updateTimeslot throws when not found', async () => {
    ...
    await expect(service.updateTimeslot(99, { capacity: 1 } as any)).rejects.toThrow(
      'Timeslot not found',
    );
  });
  ...
  it('updateTimeslot maps start_at/end_at when provided', async () => {
    ...
    timeslotsRepo.findOne.mockResolvedValueOnce(slot);
    timeslotsRepo.save.mockImplementation(async (s: any) => s);
    const res = await service.updateTimeslot(8, {
      start_at: '2025-03-01',
      end_at: '2025-03-02',
    } as any);
