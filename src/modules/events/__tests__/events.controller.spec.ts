...
describe('EventsController', () => {
  ...
  beforeEach(() => {
    ...
    controller = new EventsController(
      ...
      publicSchedule as unknown as PublicScheduleService,
    );
  });

  it('list parses active query and delegates to service', async () => {
    await controller.list('true', '2025-01-01', '2025-01-31');
    expect(eventsService.list).toHaveBeenCalledWith({
      active: true,
      from: '2025-01-01',
      to: '2025-01-31',
    });
    await controller.list(undefined as any, undefined as any, undefined as any);
    expect(eventsService.list).toHaveBeenCalledWith({
      active: undefined,
      from: undefined,
      to: undefined,
  ...
  it('create/update/remove delegate to service', async () => {
    await controller.create({ name: 'E' } as any);
    await controller.update(2, { name: 'U' } as any);
  ...
  it('timeslot methods delegate and attach event id', async () => {
    await controller.listTimeslots(9);
    await controller.createTimeslot(9, { capacity: 10 } as any);
    await controller.updateTimeslot(7, { capacity: 5 } as any);
    await controller.removeTimeslot(7);
    expect(eventsService.listTimeslots).toHaveBeenCalledWith(9);
    expect(eventsService.createTimeslot).toHaveBeenCalledWith(
