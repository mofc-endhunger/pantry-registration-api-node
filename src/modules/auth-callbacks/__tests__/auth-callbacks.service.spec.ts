...
describe('AuthCallbacksService', () => {
  ...
  beforeEach(async () => {
    ...
    jest.spyOn(service as any, 'verifyFacebookToken').mockReturnValue(true);
  });

  it('throws when token verification fails', async () => {
    (service as any).verifyFacebookToken.mockReturnValueOnce(false);
    await expect(
      service.facebookCallback({ userID: 'u', graphDomain: 'fb', accessToken: 'bad' } as any),
  ...
  it('uses existing identity and loads user', async () => {
    ...
    const res = await service.facebookCallback({
      userID: 'u1',
      graphDomain: 'fb',
      accessToken: 't',
    } as any);
  ...
  it('throws when identity exists but user not found', async () => {
    ...
    await expect(
      service.facebookCallback({ userID: 'u2', graphDomain: 'fb', accessToken: 't' } as any),
    ).rejects.toThrow('User not found for identity');
  });
