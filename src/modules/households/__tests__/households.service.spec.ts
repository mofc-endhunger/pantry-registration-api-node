...
describe('HouseholdsService', () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(Household), useValue: createRepoMock<Household>() },
        {
          ...
          useValue: createRepoMock<HouseholdMember>(),
        },
        {
          provide: getRepositoryToken(HouseholdAddress),
          useValue: createRepoMock<HouseholdAddress>(),
        },
  ...
  it('createHousehold saves household and primary member then returns computed counts', async () => {
    ...
    householdsRepo.findOne
      ...
      .mockResolvedValue({
        id: 10,
        added_by: 1,
        members: [{ id: 20, is_active: true, date_of_birth: '1980-01-01' }],
      } as any);
    const res = await service.createHousehold(1, {
      primary_first_name: 'A',
      primary_last_name: 'B',
    } as any);
  ...
  it('getHouseholdForPrimary returns computed counts', async () => {
    householdsRepo.findOne.mockResolvedValueOnce({
      id: 5,
      added_by: 7,
      members: [
    ...
    expect((res as any).counts).toEqual(
  ...
  it('updateHousehold updates fields, writes address history, upserts members, and returns via getHouseholdById', async () => {
    ...
    const spyGet = jest
      .spyOn(service as any, 'getHouseholdById')
      .mockResolvedValue({ id: 11 } as any);
    await service.updateHousehold(11, 1, {
      name: 'New',
      identification_code: 'ABC',
      deleted_on: '2025-01-01',
      line_1: '123',
      city: 'Town',
      state: 'OH',
      zip_code: '43085',
    ...
    expect(householdsRepo.save).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'New', identification_code: 'ABC' }),
    );
    expect(addressesRepo.update).toHaveBeenCalled();
    expect(addressesRepo.save).toHaveBeenCalledWith(
      expect.objectContaining({ household_id: 11, line_1: '123' }),
    );
  ...
  it('updateHousehold only writes new address when changed and sets deleted_by on soft-delete', async () => {
    ...
    const spyGet = jest
      .spyOn(service as any, 'getHouseholdById')
      .mockResolvedValue({ id: 13 } as any);

    // No-op: provide same address -> should NOT update/save
    await service.updateHousehold(13, 3, {
      line_1: '123 Main',
      line_2: 'Apt 1',
      city: 'Town',
      state: 'OH',
  ...
  it('addMember validates ownership and saves', async () => {
    ...
    const res = await service.addMember(1, 1, {
      first_name: 'A',
      last_name: 'B',
      date_of_birth: '2000-01-01',
    } as any);
