...
describe('AuthService.registerGuest', () => {
  ...
  beforeEach(async () => {
    ...
    usersRepo.save.mockImplementation(async (u: any) => {
      u.id = 1;
      return u;
    });
    authRepo.save.mockImplementation(async (a: any) => {
      a.id = 2;
      return a;
    });
  });
  ...
  it('creates guest user and authentication with token', async () => {
    ...
    expect(authRepo.save).toHaveBeenCalledWith(
      expect.objectContaining({ user_id: expect.any(Number), token: expect.any(String) }),
    );
    expect(res).toEqual(
      expect.objectContaining({ id: expect.any(Number), token: expect.any(String) }),
    );
  });
});
