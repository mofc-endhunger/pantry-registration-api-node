...
describe('AuthService.resetPassword', () => {
  ...
  it('throws for missing token', async () => {
    ...
    await expect(service.resetPassword({ token: 'bad', newPassword: 'x' } as any)).rejects.toThrow(
      'Invalid or expired token',
    );
  });

  it('throws for expired token', async () => {
    resetRepo.findOne.mockResolvedValueOnce({
      id: 1,
      user_id: 1,
      expires_at: new Date(Date.now() - 1000),
    } as any);
    await expect(service.resetPassword({ token: 't', newPassword: 'x' } as any)).rejects.toThrow(
      'Invalid or expired token',
    );
  });
  ...
  it('creates credential if missing and deletes token on success', async () => {
    ...
    expect(credRepo.save).toHaveBeenCalledWith(
      expect.objectContaining({ user_id: 3, secret: expect.any(String) }),
    );
