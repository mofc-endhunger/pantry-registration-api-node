...
describe('GuestAuthenticationsController', () => {
  ...
  beforeEach(() => {
    service = {
      createGuest: jest.fn().mockResolvedValue({ id: 1 }),
      updateGuestByToken: jest.fn().mockResolvedValue({ id: 2 }),
    } as any;
    controller = new GuestAuthenticationsController(
      service as unknown as GuestAuthenticationsService,
    );
  });
  ...
  it('update requires X-Guest-Token header', async () => {
    await expect(
      controller.update({ headers: {} } as unknown as Request, { phone: '555' } as any),
